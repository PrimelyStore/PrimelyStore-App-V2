...
async function carregarMapeamento(
  ...
  const { data, error } = await userClient
    ...
    .select(
      'id, produto_id, canal_venda_id, marketplace, seller_sku, marketplace_id, is_amazon_fulfilled, status, manual_override'
    )
    .eq('id', mapeamentoId)
    .maybeSingle()
...
function validarMapeamentoAmazon(mapeamento: MarketplaceMapping) {
  ...
  if (!mapeamento.marketplace_id?.trim()) {
    throw new AppError('marketplace_id e obrigatorio para cotacao Amazon.', 400)
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const body = validarBody(await lerJsonBody(req))

    await validarPermissaoFinanceira(auth.userClient)

    // Se service_role for necessario em fases futuras, ele so pode ser
